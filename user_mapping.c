...
#include <stdlib.h>
#include <string.h>
#define CATEGORY_CSV "./Datasets/documents_topics.csv"
...
int insert_node(node *p, int category, double confidence){
...
int insert_dictionary(dict *p, char *key, int category, double confidence, int level){
	if(level < 14){
		if(p->next[*key] == NULL){
...
int main(){
	...
	double user_ka_category_correlation, user_ka_topic_correlation;
...
//	}
	...
	scanf("%s\n", c);
	while(scanf("%d,%[^,],%[^,],%d,%[^,],%[^,],%[^,],%[^,],%d,%[^,],%[^,],%[^,],%[^,],%[^,],%lf,%lf,%lf,%lf,%lf\n",
				&display_id,
				ad_id,
				clicked,
				&ad_document_id,
				...
				&user_ka_topic_correlation) != -1){
		printf("%d,%s,%s,%d,%s,%s,%s,%s,%d,%s,%s,%s,%s,%s,%.12lf,%.12lf,%.12lf,%.12lf,",
				display_id,
				ad_id,
				clicked,
				ad_document_id,
